...
export default function GPVideoCallPage() {
  ...
  const initializeCall = async () => {
    try {
      ...
      const consultationRes = await fetch(`/api/consultations/provider?providerId=5e8115c5-7b46-421b-b136-f4d029568d1c`);
  ...
  const handleLeave = () => {
    router.push(`/gp/consultations/${consultationId}`);
  };
    ...
    return (
      <div className="h-screen w-screen bg-gray-900 flex flex-col items-center justify-center">
        <div className="max-w-md bg-gray-800 rounded-lg p-8 text-center">
          ...
          <button
            onClick={() => router.push(`/gp/consultations/${consultationId}`)}
